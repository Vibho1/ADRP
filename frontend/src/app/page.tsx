...
export default function Home() {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      fetch(`http://localhost:3000/gateway/history?email=${session.user.email}`)
        .then((res) => res.json())
        .then((data) => {
          if (data && data.length > 0) {
            setMessages(data); // Instantly load past chats into the UI!
          }
        })
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/gateway/prompt", {
        ...
        body: JSON.stringify({ 
          ...
          userEmail: session?.user?.email || "anonymous" // <-- Pass the email!
        }),
      });
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white flex flex-col items-center p-4 sm:p-8">
      <div className="w-full max-w-4xl flex flex-col h-[90vh]">
        ...
        {/* Top Navigation */}
        <div className="w-full flex justify-end py-4 shrink-0">
          {session ? (
